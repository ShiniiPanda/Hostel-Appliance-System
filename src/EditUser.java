...
public class EditUser {
    ...
    private int changes = 7;
    private String changesMessage = "";

    public EditUser(Manager user, User systemUser) {
        ...
        submitButton.addActionListener(e -> {
            if (calculateChanges() == 0) {
                if (validateInput() == 0) {
                    int confirmation = JOptionPane.CANCEL_OPTION;
                    confirmation = JOptionPane.showConfirmDialog(null, "Would you like to submit the following changes:" + changesMessage,
                            "Edit Confirmation", JOptionPane.YES_NO_OPTION);
                    if (confirmation == JOptionPane.YES_OPTION) {
                        User.updateUser(compileNewData());
                        logEdit(user.getName());
                        frame.dispose();
                    }
                }
            } else {
                JOptionPane.showMessageDialog(null, "There are no changes to submit!", "Edit Failure!", JOptionPane.ERROR_MESSAGE);
            }
        });
        cancelButton.addActionListener(e -> {
            if (calculateChanges() == 0) {
                if (JOptionPane.showConfirmDialog(null, "There are changes that have not been submitted\nAre you sure you want to cancel?",
    ...
    private void populateChosenUserData() {
        String userRole = chosenUser.getRole();
        String userDOB = chosenUser.getDOB();
        String[] dateArgs = userDOB.split("/");
        usernameField.setText(chosenUser.getId());
        usernameField.setEditable(false);
        passwordField.setText(chosenUser.getPassword());
        nameField.setText(chosenUser.getName());
        emailField.setText(chosenUser.getEmail());
        ...
        yearComboBox.setSelectedItem(dateArgs[0]);
        monthComboBox.setSelectedItem(dateArgs[1]);
    ...
    private void $$$setupUI$$$() {
        ...
        label1.setText("Edit System User");
